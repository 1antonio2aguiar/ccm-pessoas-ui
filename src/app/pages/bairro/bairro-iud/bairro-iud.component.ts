...
@Component({
  selector: 'ngx-bairro-iud',
  templateUrl: './bairro-iud.component.html',
  styleUrls: ['./bairro-iud.component.scss'],
})
export class BairroIudComponent {
  ...
  listar() {
    this.service.pesquisar(this.filtro)
      .then((response) => {
        const bairros = response.bairros ?? [];
        this.source.load(bairros);
      })
      .catch((error) => console.error('Erro ao listar bairros:', error));
  }
  ...
  onCreateConfirm(event) {
    ...
    this.service.create(event.newData).subscribe({
      next: () => {
        ...
        this.toastrService.show(
          'Novo bairro cadastrado com sucesso!',
          'Cadastro Realizado',
  ...
  onSaveConfirm(event) {
    ...
    event.newData.nomeDistrito = event.data.nomeDistrito;
    event.newData.nomeCidade = event.data.nomeCidade;
    ...
    this.service.update(event.newData).subscribe({
      next: () => {
        ...
        event.confirm.resolve();
        this.toastrService.show(
          `Bairro "${event.newData.nome}" foi atualizado com sucesso!`,
  ...
  onDeleteConfirm(event): void {
    ...
    this.dialogService.open(ConfirmationDialogComponent, {
      context: {
        ...
        message: `Você tem certeza que deseja excluir o bairro <strong>"${item.nome}"</strong>?`,
      ...
      if (confirmado) {
        this.service.delete(item.id).subscribe({
          next: () => {
            ...
            event.confirm.resolve();
            this.toastrService.show(
              `Bairro "${item.nome}" foi excluído com sucesso.`,
              'Exclusão Realizada',
          ...
          error: (error) => {
            ...
            event.confirm.reject();
            this.toastrService.show(
              'Não foi possível excluir o bairro. Verifique se ele não está sendo usado em outras partes do sistema.',
              'Erro na Exclusão',
